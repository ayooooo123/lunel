...
export function useApi() {
  ...
  const fs = {
     ...
     * Write file contents
     */
    write: useCallback(async (path: string, content: string, encoding: 'utf8' | 'base64' = 'utf8'): Promise<void> => {
  ...
  const git = {
     ...
     * Pull from remote
     */
    pull: useCallback(async (): Promise<{ success: boolean; summary: string }> => {
      const response = await sendControl('git', 'pull');
     ...
     * Push to remote
     */
    push: useCallback(async (setUpstream: boolean = false): Promise<void> => {
      const response = await sendControl('git', 'push', { setUpstream });
