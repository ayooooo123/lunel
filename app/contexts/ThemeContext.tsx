...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const [selectedTheme, setSelectedTheme] = useState<ThemeOption>("device");
    ...
    try {
      ...
      if (savedFonts) {
        try {
          ...
          setFontSelection({ ...DEFAULT_FONT_SELECTION, ...parsed });
        } catch (e) {
  ...
  const themeId: ThemeId = useMemo(() => {
    if (selectedTheme === "device") {
      return resolveDeviceTheme(deviceColorScheme);
    }
    return selectedTheme;
  }, [selectedTheme, deviceColorScheme]);
